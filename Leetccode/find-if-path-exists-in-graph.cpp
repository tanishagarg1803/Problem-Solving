...
std::vector<bool>& isVisited,
    const int& source, const int& destination)
    {
        ...
        {;
            ...
            else if (findIfPathExistsInGraphDfs(adjacencyList, isVisited,
 adjacencyList[source][i], destination))
            {
                returnVal = true;
                break;
            }
   ...
   //RUNTIME: 1271 - 1348 MS
   //TIME COMPLEXITY: O(e), where e is the number of edges in the graph
   //SPACE COMPLEXITY: O(e), where e is thee number of edges in the graph
